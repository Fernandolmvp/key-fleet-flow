-- Extend consume_ai_tokens to accept provider/model UUIDs, fallback flag and response time
CREATE OR REPLACE FUNCTION public.consume_ai_tokens(
  ...
  _model_id_used uuid DEFAULT NULL,
...
BEGIN
  ...
  IF (COALESCE(b.plan_tokens_remaining, 0) + COALESCE(b.extra_tokens_balance, 0)) < _tokens_used THEN
    INSERT INTO public.ai_usage_logs(
      ...
      provider_id, model_id_used, was_fallback, response_time_ms
    )
    VALUES (
      ...
      _provider_id, _model_id_used, COALESCE(_was_fallback, false), _response_time_ms
    )
