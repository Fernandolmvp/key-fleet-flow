...
BEGIN
  ...
  -- idempotência: se já existe log com este request_id, retorna sem debitar de novo
  ...
  IF (b.plan_tokens_remaining + b.extra_tokens_balance) < _tokens_used THEN
    INSERT INTO public.ai_usage_logs(company_id,user_id,feature,model,tokens_input,tokens_output,tokens_total,source,success,error_message,request_id)
    VALUES (_company_id,_user_id,_feature,_model,_tokens_input,_tokens_output,_tokens_used,'blocked',false,'insufficient_tokens',_request_id)
  ...
  INSERT INTO public.ai_usage_logs(company_id,user_id,feature,model,tokens_input,tokens_output,tokens_total,source,success,error_message,request_id)
  VALUES (_company_id,_user_id,_feature,_model,_tokens_input,_tokens_output,_tokens_used,src,_success,_error,_request_id)
  ON CONFLICT (request_id) DO NOTHING;
...
-- Função leve para verificar saldo sem consumir (uso pelo guard antes da chamada IA)
CREATE OR REPLACE FUNCTION public.has_enough_ai_tokens(_company_id uuid, _required integer)
RETURNS boolean
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $$
  SELECT COALESCE(plan_tokens_remaining,0) + COALESCE(extra_tokens_balance,0) >= COALESCE(_required, 0)
  FROM public.ai_token_balance WHERE company_id = _company_id
  UNION ALL SELECT (0 >= COALESCE(_required, 0))
  WHERE NOT EXISTS (SELECT 1 FROM public.ai_token_balance WHERE company_id = _company_id)
  LIMIT 1;
$$;
