...
DECLARE
  ...
  v_drivers int := 0;
  v_drivers_active int := 0;
  v_cnh_expiring int := 0;
  v_trips_running int := 0;

  v_docs_expiring int := 0;
  v_maint_7d int := 0;
  v_fines_open int := 0;
  v_approvals_pending int := 0;
  ...
  v_month_liters numeric := 0;
  v_month_km_l_sum numeric := 0;

  v_prev_total numeric := 0;
  v_prev_km numeric := 0;
  v_prev_liters numeric := 0;
  v_prev_km_l_sum numeric := 0;
...
BEGIN
  ...
  FROM fuel_records WHERE company_id = p_company_id AND fueled_at >= v_30d_ago;
  ...
  WHERE company_id = p_company_id
    AND expires_at IS NOT NULL AND expires_at <= v_in30
    AND status IN ('vencido','vencendo');

  -- Manutenções próximos 7d
  SELECT count(*) INTO v_maint_7d
  ...
  -- Multas em aberto
  SELECT count(*) INTO v_fines_open
  FROM traffic_fines
  WHERE company_id = p_company_id
    AND status NOT IN ('paga','cancelada','arquivada');

  -- Aprovações pendentes (autorizações abastecimento + OS aguardando)
  SELECT
    COALESCE((SELECT count(*) FROM fuel_authorizations
              WHERE company_id = p_company_id AND status = 'pendente'),0)
    + COALESCE((SELECT count(*) FROM maintenance_work_orders
                WHERE company_id = p_company_id AND status IN ('aguardando_aprovacao','enviada')),0)
  INTO v_approvals_pending;

  -- Mês atual
  SELECT COALESCE(sum(total_value),0), COALESCE(sum(km_driven),0),
         COALESCE(sum(liters),0),
         COALESCE(sum(km_per_liter) FILTER (WHERE km_per_liter > 0),0)
  INTO v_month_fuel, v_month_km, v_month_liters, v_month_km_l_sum
  FROM fuel_records
  WHERE company_id = p_company_id AND fueled_at::date >= v_month_start;

  SELECT COALESCE(sum(total_value),0) INTO v_month_maint
  FROM maintenance_records WHERE company_id = p_company_id AND service_at::date >= v_month_start;

  SELECT COALESCE(sum(amount),0) INTO v_month_expenses
  FROM vehicle_expenses WHERE company_id = p_company_id AND expense_date >= v_month_start;

  SELECT COALESCE(sum(COALESCE(paid_amount, amount, 0)),0) INTO v_month_fines
  FROM traffic_fines WHERE company_id = p_company_id AND infraction_date >= v_month_start;

  SELECT COALESCE(sum(amount),0) INTO v_month_trip_exp
  FROM trip_expenses WHERE company_id = p_company_id AND expense_date >= v_month_start;

  -- Mês anterior (custo total + km + km/L médio)
  SELECT
  ...
  SELECT COALESCE(sum(km_driven),0), COALESCE(sum(liters),0),
         COALESCE(sum(km_per_liter) FILTER (WHERE km_per_liter > 0),0)
  INTO v_prev_km, v_prev_liters, v_prev_km_l_sum
  FROM fuel_records
  WHERE company_id = p_company_id
    AND fueled_at::date BETWEEN v_prev_month_start AND v_prev_month_end;

  -- Alertas detalhados (mantido)
  ...
  INTO v_alerts
  FROM (SELECT title, doc_type, expires_at FROM documents
  ...
  INTO v_alerts
  FROM (SELECT full_name, cnh_expires_at FROM drivers
  ...
  INTO v_alerts
  FROM (SELECT category, target_date FROM maintenance_schedules
  ...
  INTO v_upcoming
  FROM (SELECT * FROM up ORDER BY due ASC LIMIT 20) x;

  -- Série de custo últimos 12 meses
  WITH months AS (
  ...
  INTO v_series
  FROM months LEFT JOIN totals t ON t.m = months.m;

  -- Ranking km/L últimos 90 dias (soma km / soma litros por veículo)
  WITH fr AS (
    SELECT vehicle_id, sum(km_driven) AS km, sum(liters) AS lt
    FROM fuel_records
    WHERE company_id = p_company_id AND fueled_at >= v_90d_ago
  ...
  RETURN jsonb_build_object(
    ...
    'generated_at', v_now,
    'mode', CASE WHEN v_total_vehicles < 3 THEN 'new' ELSE 'active' END,
