...
import { Plus, Pencil, Trash2, Search, Package, Star, Loader2, LayoutGrid, List } from "lucide-react";
...
import { SUPPLIER_CATEGORIES, PAYMENT_TERMS, PIX_KEY_TYPES, INVOICE_TYPES_SERVICE, PARTNER_STATUS, labelOf } from "@/lib/partners";
import { onlyDigits, isValidDocument } from "@/lib/document";

const blank = () => ({
  id: "", name: "", trade_name: "",
  document_type: "cnpj" as "cnpj" | "cpf", document_number: "",
  state_registration: "", municipal_registration: "",
  supplier_category: [] as string[],
  ...
  preferred: false, credit_limit: "", discount_pct: "0",
  delivery_days_avg: "", minimum_order: "",
...
export default function Suppliers() {
  ...
  const [view, setView] = useState<"grid" | "list">(() => (localStorage.getItem("suppliers:view") as "grid" | "list") || "grid");
  useEffect(() => { localStorage.setItem("suppliers:view", view); }, [view]);
  ...
  const openEdit = (s: any) => {
    ...
    setForm({
      ...
      tags: (s.tags ?? []).join(", "),
      iss_rate: s.iss_rate?.toString() ?? "",
      icms_rate: s.icms_rate?.toString() ?? "",
      credit_limit: s.credit_limit?.toString() ?? "",
      discount_pct: s.discount_pct?.toString() ?? "0",
      delivery_days_avg: s.delivery_days_avg?.toString() ?? "",
      minimum_order: s.minimum_order?.toString() ?? "",
    });
    setOpen(true);
  };
  ...
  const toggleCat = (v: string) =>
    setForm((f: any) => ({
      ...f,
      supplier_category: f.supplier_category.includes(v)
        ? f.supplier_category.filter((x: string) => x !== v)
        : [...f.supplier_category, v],
    }));
  ...
  const save = async () => {
    ...
    const payload: any = {
      ...
      delivery_days_avg: form.delivery_days_avg ? Number(form.delivery_days_avg) : null,
  ...
  const filtered = items.filter((s) => {
    ...
    return (s.name + " " + (s.trade_name ?? "") + " " + (s.document_number ?? "") + " " + (s.city ?? "")).toLowerCase().includes(t);
  });
    ...
    <div className="space-y-6">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((s) => {
            const tone = PARTNER_STATUS.find((p) => p.value === s.status)?.tone ?? "";
            return (
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="font-display font-semibold truncate flex items-center gap-1.5">
                      {s.name}
                      {s.preferred && <Star className="h-3.5 w-3.5 text-warning fill-warning" />}
                    </div>
                    {s.trade_name && <div className="text-xs text-muted-foreground">{s.trade_name}</div>}
                  </div>
                  <Badge className={`border ${tone}`}>{labelOf(PARTNER_STATUS, s.status)}</Badge>
                </div>
                {s.document_number && <div className="font-mono text-xs text-muted-foreground">{s.document_number}</div>}
                {(s.city || s.state) && <div className="text-xs text-muted-foreground">{[s.city, s.state].filter(Boolean).join(" / ")}</div>}
                {s.supplier_category?.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {s.supplier_category.slice(0, 4).map((t: string) => (
                      <span key={t} className="text-[10px] px-1.5 py-0.5 rounded bg-primary/10 text-primary border border-primary/20">{labelOf(SUPPLIER_CATEGORIES, t)}</span>
                    ))}
                    {s.supplier_category.length > 4 && <span className="text-[10px] text-muted-foreground">+{s.supplier_category.length - 4}</span>}
                  </div>
                )}
                ...
                {filtered.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className="border-t border-border hover:bg-muted/20">
                      ...
                      <td className="px-4 py-3 font-mono text-xs">{s.document_number || "—"}</td>
                      <td className="px-4 py-3 text-xs">{[s.city, s.state].filter(Boolean).join(" / ") || "—"}</td>
                      <td className="px-4 py-3 text-xs text-muted-foreground">
                        {s.supplier_category?.length ? s.supplier_category.slice(0, 3).map((t: string) => labelOf(SUPPLIER_CATEGORIES, t)).join(", ") + (s.supplier_category.length > 3 ? ` +${s.supplier_category.length - 3}` : "") : "—"}
                      </td>
                      <td className="px-4 py-3"><Badge className={`border ${tone}`}>{labelOf(PARTNER_STATUS, s.status)}</Badge></td>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
          <DialogHeader><DialogTitle className="font-display text-2xl">{editing ? "Editar fornecedor" : "Novo fornecedor"}</DialogTitle></DialogHeader>
          <Tabs defaultValue="ident">
            ...
            <TabsContent value="ident" className="space-y-4 mt-4">
              ...
              <div className="space-y-2">
                <Label>Categorias do fornecedor</Label>
                <div className="flex flex-wrap gap-1.5">
                  {SUPPLIER_CATEGORIES.map((t) => {
                    const on = form.supplier_category.includes(t.value);
                    return (
                      <button key={t.value} type="button" onClick={() => toggleCat(t.value)}
            ...
            <TabsContent value="commercial" className="space-y-4 mt-4">
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2"><Label>Dias médios de entrega</Label>
                  <Input type="number" value={form.delivery_days_avg} onChange={(e) => setForm({ ...form, delivery_days_avg: e.target.value })} />
                </div>
                <div className="space-y-2"><Label>Pedido mínimo (R$)</Label>
                  <Input type="number" step="0.01" value={form.minimum_order} onChange={(e) => setForm({ ...form, minimum_order: e.target.value })} />
                </div>
            ...
            <TabsContent value="contract" className="space-y-4 mt-4">
              ...
              <div className="space-y-2"><Label>Tags</Label>
                <Input placeholder="Ex.: confiável, rápido" value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} />
